...
**책임 분리를 통한 아키텍처 설계**
초기 요구사항 분석 단계에서부터 각 기능의 책임(Responsibility)을 명확히 분리하여 계층형 아키텍처를 구성했습니다.

   - `domain`: 로또, 당첨 번호, 등수 등 게임의 핵심 규칙과 데이터를 포함하는 객체들을 모았습니다. 이들은 다른
     어떤 계층에도 의존하지 않는 순수한 비즈니스 로직의 집합체입니다.

   - `view`: 사용자의 입력을 받고 결과를 출력하는 모든 로직을 포함합니다. Console에 대한 의존성을 이 계층으로
     한정함으로써, 향후 UI가 웹으로 변경되더라도 domain이나 service 로직의 수정 없이 view만 교체하여 대응할 수
      있는 유연한 구조를 만들었습니다.

   - `service`: 도메인 객체들을 조합하여 애플리케이션의 주요 비즈니스 로직을 수행합니다. 로또 발행, 당첨 통계
     계산, 수익률 계산 등 특정 도메인 하나에 속하기 어려운 기능들을 서비스 계층으로 분리하여 도메인 객체가
     비대해지는 것을 막았습니다.

   - `controller`: InputView로부터 입력을 받아 service에 처리를 위임하고, 그 결과를 OutputView로 전달하는 등
     전체 애플리케이션의 흐름을 조율하는 역할만 수행합니다. 컨트롤러는 비즈니스 로직을 직접 포함하지 않고
     오케스트레이션에만 집중하여 코드를 이해하기 쉽게 만들었습니다.

   - `validator`, `constants`: 입력값 검증 로직과 상수들을 별도의 패키지로 분리하여 재사용성을 높이고, 코드의
     응집도와 가독성을 향상시켰습니다.

**코드 구현 방식의 특징**
이번 과제에서 특히 신경 쓴 몇 가지 구현 방식이 있습니다.

- 상태를 갖지 않는 서비스(Stateless Services): `LottoMachine`, `WinningService` 등 대부분의 서비스 객체를 class가 아닌 일반 객체 리터럴로 구현했습니다. 이들은 내부 상태를 갖지 않고 입력된 값에 대해서만 동작하는 순수 함수에 가까운 형태로, 코드의 동작을 예측하기 쉽고 테스트를 단순하게 만듭니다.

- 방어적인 입력 처리: `InputView`에서 `while(true)`와 `try-catch`를 사용하여 유효한 값이 입력될 때까지 재입력을 받도록 구현했습니다. 특히, 단순 유효성 검사 오류([ERROR]로 시작)와 테스트 환경의 입력 스트림 종료(undefined 반환)와 같은 치명적 오류를 구분하여 처리함으로써, 예기치 않은 상황에서도 무한 루프에 빠지지 않는 견고한 코드를 작성할 수 있었습니다.

- 선언적인 코드 스타일: `Rank.of` 메서드에서 여러 `if `문을 사용하는 대신 조회 맵(mapping object)을 사용하거나, `forEach`와 `if`의 조합을 `filter`와 `forEach`로 변경하는 등, 코드의 실행 방식보다는 무엇을 하는지(intent)가 명확히 드러나는 선언적인 코드 작성을 지향했습니다.
